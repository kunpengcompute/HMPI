    ...
    ADIO_Offset *fd_size_ptr, void *fs_ptr)
{
    ...
    ADIO_Offset oceanfs_ub = (max_end_offset + blksize - 1) / blksize * blksize - 1;
    ADIO_Offset oceanfs_lb = min_st_offset / blksize * blksize;
    ADIO_Offset oceanfs_ub_rdoff = (max_end_offset + blksize - 1) / blksize * blksize - 1 - max_end_offset;
    ADIO_Offset oceanfs_lb_rdoff = min_st_offset - min_st_offset / blksize * blksize;
    ADIO_Offset fd_oceanfs_range = oceanfs_ub - oceanfs_lb + 1;
    ...
    *fd_end_ptr = (ADIO_Offset *)ADIOI_Malloc(nprocs_for_coll * sizeof(ADIO_Offset));
    ...
    ADIO_Offset naggs_large = n_oceanfs_blk - naggs * (n_oceanfs_blk / naggs);
    ADIO_Offset fd_size_small = (nb_cn_small + 1) * blksize;
    ADIO_Offset fd_size_large = nb_cn_small * blksize;

    for (i = 0; i < naggs; i++) {
        if (i < naggs_large) {
            fd_size[i] = fd_size_small;
        } else {
            fd_size[i] = fd_size_large;
        }
    }

    fd_size[0] -= oceanfs_lb_rdoff;
    fd_size[naggs - 1] -= oceanfs_ub_rdoff;
