...
static void mca_coll_ucg_init_group_param(struct ompi_communicator_t *comm, ucg_group_params_t *args)
{
    ...
    args->mpi_dt_is_predefine = ompi_datatype_is_predefined;
...
{
    unsigned pps = mca_coll_ucx_calculate_ppx(group_params, UCG_GROUP_MEMBER_DISTANCE_SOCKET);
    unsigned ppn = mca_coll_ucx_calculate_ppx(group_params, UCG_GROUP_MEMBER_DISTANCE_HOST);
...
static int mca_coll_ucg_create(mca_coll_ucx_module_t *module, struct ompi_communicator_t *comm)
{
    ...
    args.is_socket_balance = args.topo_args.rank_balance_in_sock;
...
#define UCX_SAVE_PREV_COLL_API(__api) do {\
        ucx_module->previous_ ## __api            = comm->c_coll->coll_ ## __api;\
        ucx_module->previous_ ## __api ## _module = comm->c_coll->coll_ ## __api ## _module;\
        if (!comm->c_coll->coll_ ## __api || !comm->c_coll->coll_ ## __api ## _module) {\
            return OMPI_ERROR;\
        }\
        OBJ_RETAIN(ucx_module->previous_ ## __api ## _module);\
} while(0)

static int mca_coll_ucx_save_coll_handlers(mca_coll_ucx_module_t *ucx_module)
{
    ompi_communicator_t *comm;
    comm = ucx_module->comm;

    UCX_SAVE_PREV_COLL_API(allreduce);
    UCX_SAVE_PREV_COLL_API(barrier);
    UCX_SAVE_PREV_COLL_API(bcast);
    UCX_SAVE_PREV_COLL_API(alltoallv);
                                      ...
                                      struct ompi_communicator_t *comm)
{
    ...
    rc = mca_coll_ucx_save_coll_handlers(ucx_module);
    if (rc != OMPI_SUCCESS) {
        COLL_UCX_ERROR("Failed to save previous coll handlers: %d", rc);
        return rc;
    }
