...
{
#ifdef HAVE_UCP_REQUEST_PARAM_T
    coll_ucx_datatype_t *ucp_type = (coll_ucx_datatype_t*)datatype->coll_data;

    if (OPAL_LIKELY(ucp_type != COLL_UCX_DATATYPE_INVALID)) {
        return ucp_type->datatype;
    }
#else
    ucp_datatype_t ucp_type = datatype->coll_data;
...
mca_coll_ucx_get_op_data(ompi_datatype_t *datatype)
{
    ...
    return (coll_ucx_datatype_t*)datatype->coll_data;
}
